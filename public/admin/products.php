...
<form action="/estoque-sorveteria/product/store" method="POST">
    ...
    <div>
        ...
        <input type="number" id="estoque_atual" name="estoque_atual" step="0.001" min="0" placeholder="0.000" required>
    </div>
    <div>
        <label for="estoque_minimo">Estoque Mínimo (kg) *</label>
        <input type="number" id="estoque_minimo" name="estoque_minimo" step="0.001" min="0" placeholder="0.000" required>
    </div>
    <div>
            ...
            <tr>
                ...
                <td>
                    ...
                    <form action="/estoque-sorveteria/product/update/<?= $prod['id'] ?>" method="POST" style="display:inline;">
                        ...
                        <input type="number" name="estoque_atual" value="<?= htmlspecialchars($prod['estoque_atual']) ?>" step="0.001" min="0" required>
                        <input type="number" name="estoque_minimo" value="<?= htmlspecialchars($prod['estoque_minimo']) ?>" step="0.001" min="0" required>
                        <input type="number" name="estoque_maximo" value="<?= htmlspecialchars($prod['estoque_maximo'] ?? '') ?>" step="0.001" min="0">
                        <input type="number" name="preco_custo" value="<?= htmlspecialchars($prod['preco_custo'] ?? '') ?>" step="0.01" min="0">
                        <input type="number" name="preco_venda" value="<?= htmlspecialchars($prod['preco_venda'] ?? '0.00') ?>" step="0.01" min="0" required>
                        <input type="number" name="margem_lucro" value="<?= htmlspecialchars($prod['margem_lucro'] ?? '') ?>" step="0.01" min="0">
                    ...
                    <?php if ($prod['ativo']): ?>
                        ...
                        <form action="/estoque-sorveteria/product/destroy/<?= $prod['id'] ?>" method="POST" style="display:inline;" onsubmit="return confirm('Tem certeza que deseja desativar este produto?');">
                            <button type="submit">Desativar</button>
                        </form>
                    <?php else: ?>
                        <!-- Formulário de Reativação para produtos inativos -->
                        <form action="/estoque-sorveteria/product/reactivate/<?= $prod['id'] ?>" method="POST" style="display:inline;">
